...
from .schemas import AnalyzeResponse, HighlightSpan, SourceResult
...
async def plagiarism_pipeline(text: str) -> tuple[float, list[HighlightSpan], list[SourceResult], dict]:
    ...
    for chunk, vector, candidates in zip(chunks, chunk_vectors, retrieved):
        ...
        ranked = rerank(chunk.text, expanded_candidates[:25], top_n=5)
            ...
            all_matches.append(
                {
                    ...
                    "matched_text": candidate.text[:600],
                }
            )
...
def ai_detection_pipeline(text: str) -> tuple[float, str, dict]:
    chunks = chunk_text(text, chunk_words=250, overlap_words=50)
    ...
    return ai_score, explanation, avg_features
...
async def analyze_document(text: str, document_id: str | None = None, filename: str | None = None) -> AnalyzeResponse:
    ...
    ai_score, explanation, ai_features = ai_detection_pipeline(cleaned)
