...
def main() -> None:
    ...
    run([python, str(root / "ml_service" / "train_roberta.py"), "--data", str(root / "data" / "processed" / "research_ai_human.csv"), "--output", str(root / "models" / "roberta-ai-detector"), "--base-model", "roberta-base", "--epochs", "3", "--batch-size", "4", "--max-length", "384"])
    run([python, str(root / "ml_service" / "train_ensemble.py"), "--data", str(root / "data" / "processed" / "research_ai_human.csv"), "--output", str(root / "models" / "ensemble.joblib")])
